...
async def perform_rag_search(
    ...
    access_token: str,
    flow: str = "C",
    department_directory: Optional[str] = None,  # 🔹 NUEVO
) -> Dict[str, Any]:
    """
    Llama al servidor MCP para invocar la tool 'rag_search_tool' y obtener
    resultados RAG.

    La tool 'rag_search_tool' a su vez llama al microservicio NLP (/search),
    por lo que desde este punto de vista la interfaz se mantiene.

    Devuelve SIEMPRE un diccionario con al menos:
      ...
      - message: mensaje opcional del backend (si la tool lo reenvía)
    ...
    Parámetros:
      - prompt:               consulta normalizada / RAG query
      - access_token:         token de sesión para el microservicio NLP
      - flow:                 "C" (Cosmos) o "R" (rápido)
      - department_directory: directorio departamental explícito (si el front lo fija)
    """
    flow_norm = (flow or "C").upper()

    # Base del MCP (reutilizamos CREW_BASE_URL que ya usas en el orquestador)
    mcp_base = (os.getenv("CREW_BASE_URL") or "http://127.0.0.1:8090/api/v1").rstrip("/")
    url = f"{mcp_base}/tools/rag_search_tool/invoke"

    # Payload que entiende la tool rag_search_tool (ver cosmos_mcp_app/tools.py)
    args: Dict[str, Any] = {
        "query": prompt,
        "flow": flow_norm,
    ...
    # 🔹 NUEVO: si el backend de negocio ha fijado un departamento, lo propagamos
    ...
    logger.info(
        "perform_rag_search: llamando a MCP tool 'rag_search_tool' → POST %s "
        "(flow=%s, department_directory=%s)",
        url,
        flow_norm,
    ...
    # La respuesta de /tools/{name}/invoke es {"result": {...payload_de_la_tool...}}
    rag_payload: Dict[str, Any] = data.get("result") or {}

    status = rag_payload.get("status") or "ok"

    logger.info(
        "perform_rag_search: status=%s, used_department=%s, cleaned_query=%s",
        status,
        rag_payload.get("used_department"),
        rag_payload.get("cleaned_query"),
    )

    # Normalizamos estructura de salida para que el resto del código no cambie
    return {
        "status": status,
        ...
        "message": rag_payload.get("message"),  # puede venir a None si la tool no lo reenvía
    }
...
def perform_rag_search_sync(
    ...
    """
    Versión SÍNCRONA de perform_rag_search, pensada para ser usada desde
    tools de CrewAI u otros componentes sync.

    MISMA semántica que la versión async: devuelve el MISMO dict con:
      - status
      - results
      - aggregation
      - used_department
    ...
    # 🔹 NUEVO: igual que en la versión async
    if department_directory:
        args["department_directory"] = department_directory
    ...
    logger.info(
        "perform_rag_search_sync: llamando a MCP tool 'rag_search_tool' → POST %s "
        "(flow=%s, department_directory=%s)",
        url,
        flow_norm,
        department_directory,
    ...
    try:
        ...
        logger.error(
            "Error HTTP llamando al MCP (rag_search_tool): %s (tipo=%s)",
            exc,
            type(exc).__name__,
        )
        raise
    ...
    try:
        ...
        logger.error(
            "Respuesta del MCP para rag_search_tool no es JSON válido. "
            "status=%s body=%s",
            resp.status_code,
            resp.text[:2000],
        )
    ...
    logger.info(
        "perform_rag_search_sync: status=%s, used_department=%s, cleaned_query=%s",
        status,
        rag_payload.get("used_department"),
        rag_payload.get("cleaned_query"),
    )
...
async def get_current_auth_chatdoc(request: Request) -> Dict[str, Any]:
    """
    Dependencia de autenticación específica para el microservicio chat_document.

    - Valida SIEMPRE el JWT (cookie 'access_token' o header Authorization) usando verify_token.
    - Recupera la sesión desde Redis (session:{user_id}) con get_session_from_redis.
    - NO aplica la regla de "sesión unívoca" (no compara el JWT actual con el token de la sesión);
      esa lógica estricta solo la usa el microservicio de modelo de negocio.
    - Devuelve una estructura compatible con la que se usa en el resto de código:
        {
          "user_id": int,
          "role": int | None,
          "token_payload": dict,
    ...
    if not user_id:
        logger.warning("[chatdoc/auth] Token sin user_id.")
        raise HTTPException(
            status_code=401,
            detail="Token inválido (sin user_id).",
        )

    # 2) Recuperar sesión desde Redis (misma clave que en modelo_negocio: session:{user_id})
    try:
        session_data = await get_session_from_redis(user_id)
    except Exception as e:
        logger.warning(
            "[chatdoc/auth] Error recuperando sesión en Redis para user_id=%s: %s",
            user_id,
            e,
        )
        session_data = None

    # Normalizamos: siempre un dict
...
async def scan_with_clamav(content: bytes, filename: Optional[str] = None) -> Dict[str, Any]:
    ...
    timeout = float(os.getenv("CLAMAV_TIMEOUT", "70.0"))
...
# utils.py (solo reemplaza estas funciones)

_DOC_TOOLS_CACHE: Dict[str, Any] = {"ts": 0.0, "tools": {}}
_DOC_TOOLS_TTL_SEC = float(os.getenv("CHATDOC_API_TOOLS_TTL_SEC", "60"))
_DOC_CREW_NAME = os.getenv("CHATDOC_MCP_CREW_NAME", "document_crew")

mcp_client = MCPClient()

def _refresh_doc_tools(force: bool = False) -> Dict[str, Dict[str, Any]]:
    """
    Descubre tools disponibles para document_crew desde MCP con caché TTL.

    - Usa list_tools(crew=...) si el gateway entiende 'crew'.
    - Si el gateway ignora el filtro, igual devuelve el listado global y
      este selector por tags/nombre seguirá funcionando.
    """
    now = time.time()
    if (
        not force
        and _DOC_TOOLS_CACHE["tools"]
        and (now - _DOC_TOOLS_CACHE["ts"]) < _DOC_TOOLS_TTL_SEC
    ):
        return _DOC_TOOLS_CACHE["tools"]

    try:
        tools = mcp_client.list_tools(crew=_DOC_CREW_NAME, auth_token=os.getenv("CHATDOC_MCP_SERVICE_TOKEN")) or []
        ...
        _DOC_TOOLS_CACHE["tools"] = tool_map
        _DOC_TOOLS_CACHE["ts"] = now

        logger.info(
            "[chatdoc/api] Tools MCP disponibles para %s: %s",
            _DOC_CREW_NAME,
            list(tool_map.keys()) or [],
        )
        return tool_map

    except Exception as e:
        logger.warning(
            "[chatdoc/api] No se pudo listar tools MCP para %s: %s",
            _DOC_CREW_NAME, e
        )
        return _DOC_TOOLS_CACHE.get("tools") or {}


def _pick_doc_tool(kind: str) -> str:
    """
    Selección no cableada por tags/nombre.

    kind: "ingest" | "query" | "summary"
    """
    tools = _refresh_doc_tools()
    kind = (kind or "").lower().strip()

    canonical = {
        "ingest": "chatdoc_ingest_tool",
        "query": "chatdoc_query_tool",
        "summary": "chatdoc_summary_tool",
    }
    can = canonical.get(kind)
    if can and can in tools:
        return can

    preferred_tag_sets = {
        "ingest": [{"chatdoc", "ingest"}, {"document", "ingest"}],
        "query": [{"chatdoc", "query"}, {"document", "query"}, {"rag", "query"}],
        "summary": [{"chatdoc", "summary"}, {"document", "summary"}],
    }

    for tag_set in preferred_tag_sets.get(kind, []):
    ...
    for name in tools.keys():
        ln = name.lower()
        if kind == "ingest" and "ingest" in ln and "chatdoc" in ln:
            return name
        if kind == "query" and "query" in ln and "chatdoc" in ln:
            return name
        if kind == "summary" and "summary" in ln and "chatdoc" in ln:
            return name

    return can or "chatdoc_query_tool"
